...
const runPlaybookActions = async ({
  runId,
  playbookId,
  incidentId,
  context = {},
  ...
  if (run.status === playbookRunStatus.PAUSED && !resume)
    return { paused: true };
  ...
  for (let index = 0; index < sortedActions.length; index += 1) {
    ...
    try {
      if (
        action.condition &&
        !playbookEngine.evaluateCondition(action.condition, ctx)
      ) {
        if (step) {
  ...
  if (
    ![playbookRunStatus.CANCELLED, playbookRunStatus.PAUSED].includes(
      finalRun.status,
    )
  ) {
    const hasFailure = results.some((r) => r.success === false);
    finalRun.status = hasFailure
...
soarQueue.process("executePlaybook", PROCESS_OPTS.concurrency, async (job) => {
  ...
  for (const action of sortedActions) {
    try {
      ...
      await Incident.findByIdAndUpdate(incidentId, {
        $push: {
          actions: { action: action.type, result: JSON.stringify(result) },
        },
      });
    } catch (err) {
