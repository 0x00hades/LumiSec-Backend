...
import {
    entityType, auditAction, connectorType, integrationActionStatus
...
export const updateConnector = async (id, updates, user) => {
    ...
    await auditUpdate(user, entityType.CONNECTOR, connector._id, oldValue, connector.toObject());
    return connector;
};
...
export const testConnector = async (id, user) => {
        ...
        switch (connector.type) {
            case connectorType.FIREWALL: {
                ...
                await axios.get(`https://${host}/api/v2/monitor/system/status`, {
                    headers: { Authorization: `Bearer ${creds.token || process.env.FORTIGATE_TOKEN}` },
                    timeout: 10000,
                    validateStatus: () => true
                });
            ...
            default:
                result = { reachable: true, type: connector.type, message: "Configuration validated" };
                success = true;
        }
    } catch (error) {
    ...
    await IntegrationAction.create({
        ...
        status: success ? integrationActionStatus.SUCCESS : integrationActionStatus.FAILED,
        request: { action: "test" },
        response: result,
        executedBy: user._id
    });
...
export const listConnectorActions = async (connectorId, query = {}) => {
    ...
    const availableActions = CONNECTOR_ACTIONS[connector.type] || CONNECTOR_ACTIONS[connectorType.CUSTOM];
    ...
    return {
        connector: { id: connector._id, name: connector.name, type: connector.type },
        availableActions,
