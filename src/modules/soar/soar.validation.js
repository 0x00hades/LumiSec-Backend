...
export const createIncidentValidation = Joi.object({
    ...
    status: Joi.string().valid(incidentStatus.NEW, incidentStatus.OPEN).optional(),
...
export const updateIncidentValidation = Joi.object({
    ...
    description: Joi.string().max(5000).optional(),
    severity: Joi.string().valid(...Object.values(incidentSeverity)).optional(),
    status: Joi.string().valid(...Object.values(incidentStatus)).optional(),
    incidentType: Joi.string().max(100).optional(),
    sourceIP: Joi.string().ip({ version: ["ipv4", "ipv6"] }).allow(null, "").optional(),
...
export const createPlaybookValidation = Joi.object({
    ...
    triggerType: Joi.string().valid(...Object.values(playbookTrigger)).default(playbookTrigger.MANUAL),
...
export const updatePlaybookValidation = Joi.object({
    ...
    description: Joi.string().max(2000).optional(),
    triggerType: Joi.string().valid(...Object.values(playbookTrigger)).optional(),
...
export const listConnectorsValidation = listQueryValidation.keys({
    type: Joi.string().valid(...Object.values(connectorType)).optional(),
    isActive: Joi.boolean().optional()
});
...
export const enrichArtifactValidation = Joi.object({
    providers: Joi.array().items(Joi.string().valid("opencti", "virustotal", "shodan", "custom")).min(1).optional()
});
...
export const createConnectorValidation = Joi.object({
    name: Joi.string().min(2).max(120).required(),
    type: Joi.string().valid(...Object.values(connectorType)).required(),
...
export const exportAnalyticsValidation = Joi.object({
    ...
    snapshotType: Joi.string().valid("full", "kpis", "incidents", "playbooks").default("full"),
    days: Joi.number().integer().min(1).max(365).default(30)
});
