import {
    Playbook, PlaybookRun, PlaybookRunStep, Incident
} from "../../../../database/index.js";
import { AppError } from "../../../utils/appError.js";
import { messages } from "../../../utils/constant/messages.js";
import {
    entityType, auditAction, playbookRunStatus, notificationType
...
export const getPlaybookById = async (id) => {
    const playbook = await Playbook.findById(id).populate("createdBy", "name email");
...
export const updatePlaybook = async (id, updates, user) => {
    ...
    if (updates.name && updates.name !== playbook.name) {
        const duplicate = await Playbook.findOne({ name: updates.name, _id: { $ne: id } });
    ...
    await auditUpdate(user, entityType.PLAYBOOK, playbook._id, oldValue, playbook.toObject());
    return playbook;
};
...
export const executePlaybook = async ({ incidentId, playbookId, context = {} }, user) => {
...
export const cancelRun = async (runId, user) => {
    ...
    emitAlert("soc_analyst", "playbook:cancelled", { runId, incidentId: run.incidentId });
