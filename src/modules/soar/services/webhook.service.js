import {
    SoarAlert, WebhookSource, Incident
} from "../../../../database/index.js";
import { AppError } from "../../../utils/appError.js";
import { messages } from "../../../utils/constant/messages.js";
import {
    alertSource, incidentSeverity, incidentStatus, entityType, auditAction, sourceModule
...
function normalizeCrowdStrike(payload) {
    ...
    return {
        externalId: detail.event_id || detail.detection_id || payload.metadata?.eventUUID,
        title: detail.title || detail.description || "CrowdStrike detection",
        description: detail.description || detail.technique || "",
        severity: mapSeverity(detail.severity || detail.severity_name || payload.severity),
...
function normalizeWazuh(payload) {
    ...
    return {
        ...
        severity: mapSeverity(rule.level >= 12 ? "critical" : rule.level >= 8 ? "high" : rule.level >= 5 ? "medium" : "low"),
...
export const ingestWebhookAlert = async ({
    ...
    if (webhookSource?.secret) {
        const valid = verifyWebhookSignature(payload, signature, webhookSource.secret);
    ...
    if (normalized.externalId) {
        const duplicate = await SoarAlert.findOne({ externalId: normalized.externalId, source });
        if (duplicate) {
    ...
    await alertQueue.add("processSoarAlert", {
        alertId: alert._id,
        source,
        createIncident
...
export const getAlertById = async (id) => {
    const alert = await SoarAlert.findById(id).populate("incidentId", "title status severity");
